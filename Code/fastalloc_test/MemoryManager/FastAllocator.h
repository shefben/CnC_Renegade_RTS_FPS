...
protected:
	...
	unsigned long	GetAllocatorIndex(size_t input);
...
inline unsigned long FastAllocatorGeneral::GetAllocatorIndex(size_t input)
{
...
inline void FastAllocatorGeneral::Free(void* _memory)
{
	if (_memory)
	{
		...
		{
#ifdef FASTALLOC_STATS
			_InterlockedExchangeAdd((long*)&ActualMemoryUsage, -(long)1 << (index + MIN_ALLOC_BIT));
#endif
