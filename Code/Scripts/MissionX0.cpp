...
DECLARE_SCRIPT(MX0_MissionStart_DME, "") //1200001
{
	...
	void Created (GameObject * /* obj */) override
	{
		...
		Vector3 drop_loc = ScriptEngine::Get_Position ( ScriptEngine::Find_Object(1200001));
		//float chin_face = ScriptEngine::Get_Facing ( ScriptEngine::Find_Object(1200001));

		ScriptEngine::Fade_Background_Music ( "renegade_intro_no_vox.mp3", 0, 1 );
		GameObject * intro = ScriptEngine::Create_Object ( "Invisible_Object", Vector3 (0, 0, 0));
		//ScriptEngine::Set_Facing(intro, chin_face);
		ScriptEngine::Attach_Script(intro, "Test_Cinematic", "X00_Intro.txt");
	}
	...
	void Action_Complete (GameObject *obj, int action_id, ActionCompleteReason /* reason */) override
	{
		if (action_id == 100001)
		{
			ScriptEngine::Start_Timer(obj, this, 2.0f, INITIAL_CONV);
		}

		if (action_id == 100002)
		{
			ScriptEngine::Start_Timer(obj, this, 5.0f, PROMPT_LOOP);
		}

		if (action_id == 100004)
		{
			ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (1200015), SEND_EM, 0, 0.0f );
		}
	...
	void Timer_Expired(GameObject * obj, int timer_id) override
	{
		...
		{
			int id = ScriptEngine::Create_Conversation("MX0_ENGINEER1_048", 97, 2000, false);
			ScriptEngine::Join_Conversation (ScriptEngine::Find_Object (engineer1), id, false, true, true );
		...
		if (timer_id == CROUCH_WANDER && ScriptEngine::Find_Object (sniper_1))
		{
			ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (engineer1), CROUCH_WANDER, sniper_1, 0.0f );
			ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (engineer2), CROUCH_WANDER, sniper_1, 0.0f );

			ScriptEngine::Start_Timer(obj, this, 3.0f, CROUCH_WANDER);
		}

		if (timer_id == SNIPER_EXCHANGE)
		{
			ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (MX0_A02_CONTROLLER_ID), MX0_A02_CUSTOM_TYPE_MAIN_STARTUP, 0, 0.0f );
		}

		if (timer_id == SNIPER_CREATE)
		{
			Vector3 sniper1_loc = ScriptEngine::Get_Position (ScriptEngine::Find_Object (1200043));
			Vector3 sniper2_loc = ScriptEngine::Get_Position (ScriptEngine::Find_Object (1200044));

			GameObject *sniper1 = ScriptEngine::Create_Object ("NOD_Minigunner_2SF", sniper1_loc);
			ScriptEngine::Attach_Script(sniper1, "M00_Soldier_Powerup_Disable", "");
			ScriptEngine::Attach_Script(sniper1, "M00_Send_Object_ID", "1200001, 1, 0.0f");
			ScriptEngine::Attach_Script(sniper1, "MX0_SniperAction", "1200045");
			ScriptEngine::Attach_Script(sniper1, "MX0_KillNotify", "");

			sniper_1 = ScriptEngine::Get_ID (sniper1);

			GameObject *sniper2 = ScriptEngine::Create_Object ("NOD_Minigunner_2SF", sniper2_loc);
			ScriptEngine::Attach_Script(sniper2, "M00_Soldier_Powerup_Disable", "");
			ScriptEngine::Attach_Script(sniper2, "M00_Send_Object_ID", "1200001, 2, 0.0f");
			ScriptEngine::Attach_Script(sniper2, "MX0_SniperAction", "1200045");
			sniper_2 = ScriptEngine::Get_ID (sniper2);
		}

		if (timer_id == SNIPER_REMINDER && ScriptEngine::Find_Object (sniper_1))
		{
			ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (1200001), 100007, 0, 0.0f );
		}

		if (timer_id == INITIAL_CONV)
		{
			ScriptEngine::Send_Custom_Event( obj, obj, 100002, 0, 0.0f );
		}

		if (timer_id == PROMPT_LOOP && count <= 1)
		{
			ScriptEngine::Send_Custom_Event( obj, obj, 100003, 0, 0.0f );
			ScriptEngine::Start_Timer(obj, this, 5.0f, PROMPT_LOOP);
		}

		if (timer_id == MOUSE_HELP)
		{
			ScriptEngine::Set_HUD_Help_Text(MX0_HELPTEXT_01, TEXT_COLOR_OBJECTIVE_PRIMARY);
		}

		if (timer_id == WSAD_HELP)
		{
			ScriptEngine::Set_HUD_Help_Text(MX0_HELPTEXT_02, TEXT_COLOR_OBJECTIVE_PRIMARY);
		}

		if (timer_id == F1_HELP)
		{
			ScriptEngine::Set_HUD_Help_Text(MX0_HELPTEXT_03, TEXT_COLOR_OBJECTIVE_PRIMARY);
		}

		if (timer_id == EQUIP)
		{
			ScriptEngine::Set_Background_Music ( "Level0_pt1_music.mp3" );
			ScriptEngine::Give_PowerUp (STAR, "POW_AutoRifle_Player", false);
			ScriptEngine::Give_PowerUp (STAR, "POW_AutoRifle_Player", false);
			ScriptEngine::Select_Weapon (STAR, "Weapon_AutoRifle_Player" );

			ScriptEngine::Attach_Script(STAR, "Havoc_Script", "");
		}
	}
	...
	void Custom (GameObject *obj, int type, intptr_t param, GameObject *sender) override
	{
		if (type == SNIPER1KILLED)
		{
			if (!SniperNotify)
			{
				int id = ScriptEngine::Create_Conversation("MX0CON018", 97, 2000, false);
				ScriptEngine::Join_Conversation (ScriptEngine::Find_Object (engineer1), id, false, true, true );
				ScriptEngine::Join_Conversation (STAR, id, true, true, true );
				ScriptEngine::Start_Conversation(id, 100018);
				ScriptEngine::Monitor_Conversation(obj, id);

				ScriptEngine::Start_Timer(obj, this, 3.0f, STAY_HERE);
			}
		...
		{
			ScriptEngine::Start_Timer(obj, this, 2.0f, CROUCH_WANDER);
		}

		if (type == START_SNIPER)
		{
			ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (sniper_1), START_SNIPER, 0, 0.0f );
			ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (sniper_2), START_SNIPER, 0, 0.0f );
		}

		if (type == MX0_A02_CUSTOM_TYPE_GET_SNIPER_01_ID)
		{
			ScriptEngine::Send_Custom_Event( obj, sender, type, sniper_1, 0.0f );
		}

		if (type == MX0_A02_CUSTOM_TYPE_GET_SNIPER_02_ID)
		{
			ScriptEngine::Send_Custom_Event( obj, sender, type, sniper_2, 0.0f );
		}
		...
		if(type == M00_SEND_OBJECT_ID)
		{
			if(param == 1)
			{
				sniper_1 = ScriptEngine::Get_ID(sender);
			}

			if(param == 2)
			{
				sniper_2 = ScriptEngine::Get_ID(sender);
				ScriptEngine::Start_Timer(obj, this, 2.0f, SNIPER_EXCHANGE);
			}
		}
...
//			GameObject * chinook = ScriptEngine::Create_Object ( "Invisible_Object", drop_loc);
//			ScriptEngine::Set_Facing(chinook, chin_face);
//			ScriptEngine::Attach_Script(chinook, "Test_Cinematic", "MX0_TroopDrop_2X.txt");


			GameObject * block1 = ScriptEngine::Create_Object ( "Large_Blocker", Vector3 (-148.071f,-31.267f,-0.306f));
			ScriptEngine::Set_Facing(block1, -30.000f);
			ScriptEngine::Set_Is_Rendered( block1, false );
			GameObject * block2 = ScriptEngine::Create_Object ( "Large_Blocker", Vector3 (-148.234f,-31.542f,1.357f));
			ScriptEngine::Set_Facing(block2, -30.000f);
			ScriptEngine::Set_Is_Rendered( block2, false );

			ScriptEngine::Start_Timer(obj, this, 0.5f, EQUIP);
			ScriptEngine::Start_Timer(obj, this, 3.0f, MOUSE_HELP);
			ScriptEngine::Start_Timer(obj, this, 6.0f, WSAD_HELP);
			ScriptEngine::Start_Timer(obj, this, 9.0f, F1_HELP);

			ScriptEngine::Start_Timer(obj, this, 1.0f, SNIPER_CREATE);

		}

		if (type == ENGINEER_RETURN)
		{
			ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (engineer1), RUN_AWAY, 0, 0.0f );
			ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (engineer2), RUN_AWAY, 0, 0.0f );

		}

		if (type == ENGINEER_CROUCH)
		{
			ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (engineer1), ENGINEER_CROUCH, 0, 0.0f );
		}

		if (type == RUNNING_CONV)
		{
			/*int id = ScriptEngine::Create_Conversation("MX0CON005", 95, 2000, false);
			ScriptEngine::Join_Conversation (ScriptEngine::Find_Object (engineer2), id, true, true, true );
			ScriptEngine::Join_Conversation_Facing (ScriptEngine::Find_Object (engineer2), id, ScriptEngine::Get_ID (STAR));
			ScriptEngine::Join_Conversation (STAR, id, true, true, true );
		...
		if (type == KILL_SNIPER)
		{
			ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (engineer1), KILL, 0, 0.0f );
			ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (engineer2), KILL, 0, 0.0f );
		}

		if (type == COUNTER)
		{
			count = param;
			ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (engineer1), COUNT_UP, count, 0.0f );
			ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (engineer2), COUNT_UP, count, 0.0f );
		}
		...
		if (type == CHECK_ENGINEER)
		{
			if (param == 1)
			{
				ScriptEngine::Send_Custom_Event( obj, sender, RETURN_ENGINEER1, engineer1, 0.0f );
			}

			if (param == 2)
			{
				ScriptEngine::Send_Custom_Event( obj, sender, RETURN_ENGINEER2, engineer2, 0.0f );
			}
		}

		if (type == 100001)
		{
			int id = ScriptEngine::Create_Conversation("MX0CON001", 99, 2000, false);
			ScriptEngine::Join_Conversation (ScriptEngine::Find_Object (engineer2), id, false, true, true );
		...
		if (type == 100002)
		{
			int id = ScriptEngine::Create_Conversation ("MX0CON002", 97, 2000, false);
			ScriptEngine::Join_Conversation (ScriptEngine::Find_Object (engineer1), id, false, true, true );
			ScriptEngine::Join_Conversation_Facing (ScriptEngine::Find_Object (engineer1), id, ScriptEngine::Get_ID (STAR));
			ScriptEngine::Join_Conversation (ScriptEngine::Find_Object (engineer2), id, false, true, true );
			ScriptEngine::Join_Conversation_Facing (ScriptEngine::Find_Object (engineer2), id, ScriptEngine::Get_ID (STAR));
			ScriptEngine::Join_Conversation (STAR, id, true, true, true );
		...
		if (type == 100003)
		{
			...
			int rand_conv3 = ScriptEngine::Get_Random_Int (0, 4);

			int id = ScriptEngine::Create_Conversation(conv3[rand_conv3], 97, 2000, false);
			ScriptEngine::Join_Conversation (ScriptEngine::Find_Object (engineer2), id, false, true, true );
			ScriptEngine::Join_Conversation_Facing (ScriptEngine::Find_Object (engineer2), id, ScriptEngine::Get_ID (STAR));
			ScriptEngine::Join_Conversation (STAR, id, true, true, true );
		...
		if (type == 100004)
		{
			int id = ScriptEngine::Create_Conversation("MX0CON004", 95, 2000, false);
			ScriptEngine::Join_Conversation (ScriptEngine::Find_Object (engineer1), id, true, true, true );
			ScriptEngine::Join_Conversation_Facing (ScriptEngine::Find_Object (engineer1), id, ScriptEngine::Get_ID (STAR));
			ScriptEngine::Join_Conversation (STAR, id, true, true, true );
			ScriptEngine::Start_Conversation(id, 100004);
			ScriptEngine::Monitor_Conversation(obj, id);

			ScriptEngine::Innate_Disable (ScriptEngine::Find_Object (engineer1));
			ScriptEngine::Innate_Disable (ScriptEngine::Find_Object (engineer2));
		}

		if (type == 100005)
		{
			int id = ScriptEngine::Create_Conversation("MX0CON005", 97, 2000, false);
			ScriptEngine::Join_Conversation (ScriptEngine::Find_Object (engineer2), id, false, true, true );
			ScriptEngine::Join_Conversation_Facing (ScriptEngine::Find_Object (engineer2), id, ScriptEngine::Get_ID (STAR));
			ScriptEngine::Join_Conversation (STAR, id, true, true, true );
			ScriptEngine::Start_Conversation(id, 100005);
		...
		if (type == 100006)
		{
			if (ScriptEngine::Find_Object (sniper_1))
			{
				ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (engineer2), SNIPER_ANIM, 0, 0.0f );

				ScriptEngine::Start_Timer(obj, this, 5.0f, SNIPER_REMINDER);

				int id = ScriptEngine::Create_Conversation("MX0CON006", 97, 2000, false);
				ScriptEngine::Join_Conversation (ScriptEngine::Find_Object (engineer2), id, false, true, true );
				ScriptEngine::Join_Conversation_Facing (ScriptEngine::Find_Object (engineer2), id, ScriptEngine::Get_ID (STAR));
				ScriptEngine::Join_Conversation (STAR, id, true, true, true );
		...
		if (type == 100007)
		{
			if (ScriptEngine::Find_Object (sniper_1))
			{
				if (curr_conv7 == 0 || curr_conv7 == 2 || curr_conv7 == 4)
				{
					...
					int id = ScriptEngine::Create_Conversation(conv7[curr_conv7], 97, 2000, false);
					ScriptEngine::Join_Conversation (ScriptEngine::Find_Object (engineer1), id, false, true, true );
					ScriptEngine::Join_Conversation (STAR, id, true, true, true );
...
DECLARE_SCRIPT (MX0_Engineer1, "Damage_multiplier:float")
{
	...
	void Created (GameObject *obj) override
	{
		...
		ScriptEngine::Start_Timer(obj, this, 4.0f, STATIONARY);
		ScriptEngine::Start_Timer(obj, this, 4.0f, POINT_ANIM);

		ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (1200001), SET_ENGINEER, ScriptEngine::Get_ID (obj), 0.0f );
	}
	...
	void Timer_Expired(GameObject * obj, int timer_id) override
	{
		if (timer_id == RESEND_GOTO)
		{
			...
			params.Set_Movement( ScriptEngine::Get_Position (ScriptEngine::Find_Object (point_id)), RUN, 1.0f );
		...
		{
			/*ScriptEngine::Set_Facing ( obj, ScriptEngine::Get_Facing (ScriptEngine::Find_Object (1200006)));
			ActionParamsStruct params;
			...
			params.Set_Basic( this, 96, ENGINEER_GOTO );
			params.Set_Movement( ScriptEngine::Get_Position (ScriptEngine::Find_Object (1200029)), RUN, 0.8f );
	...
	void Action_Complete (GameObject *obj, int action_id, ActionCompleteReason reason) override
	{
		...
		if((action_id == ENGINEER_GOTO) && (action_id != ScriptEngine::Get_Action_ID(obj)) && (!ScriptEngine::Is_Performing_Pathfind_Action(obj)) && (reason == ACTION_COMPLETE_LOW_PRIORITY))
		{
		...
		{
			ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (1200001), 100004, 0, 0.0f );
		}
	}
	...
	void Damaged( GameObject * obj, GameObject * damager, float /* amount */ ) override
	{
		...
		if (damager == STAR && !doing_anim)
		{
			...
			int rand_conv = ScriptEngine::Get_Random_Int (0, 4);

			int id = ScriptEngine::Create_Conversation(convs [rand_conv], 97, 2000, false);
			ScriptEngine::Join_Conversation (obj, id, false, true, true );
			ScriptEngine::Join_Conversation (STAR, id, true, true, true );
	...
	void Custom(GameObject * obj, int type, intptr_t param, GameObject * /* sender */) override
	{
		if (type == CROUCH_WANDER)
		{
			GameObject * sniper1 = ScriptEngine::Find_Object (param);

			ScriptEngine::Unlock_Soldier_Facing (obj);

			if (sniper1)
			{
				ScriptEngine::Lock_Soldier_Facing (obj, sniper1, true );
				...
				int rand_goto = ScriptEngine::Get_Random_Int (0, 3);

				ActionParamsStruct params;

				params.Set_Basic( this, 97, CROUCH_GOTO );
				params.Set_Movement( ScriptEngine::Find_Object (crouch_goto [rand_goto]), WALK, 0.2f, true );
		...
		if (type == RUN_AWAY)
		{
			...
			params.Set_Movement( ScriptEngine::Find_Object (1200006), RUN, 1.0f, false );
			ScriptEngine::Action_Goto ( obj, params );
		}

		if (type == ENGINEER_CROUCH)
		{
			ScriptEngine::Lock_Soldier_Facing (obj, STAR, true );
		...
		if (type == KILL)
		{
			...
			params.Set_Movement( ScriptEngine::Find_Object(1200021), 1.2f, 1.0f, true );
			params.Set_Attack (ScriptEngine::Find_Object(1200017), 250.0f, 0.0f, 1);
		...
		if (type == LEAD)
		{
			...
			params.Set_Basic( this, 96, ENGINEER_GOTO );
			params.Set_Movement( ScriptEngine::Get_Position (ScriptEngine::Find_Object (point_id)), RUN, 0.8f );
...
DECLARE_SCRIPT (MX0_Engineer2, "Damage_multiplier:float")
{
	...
	void Created (GameObject *obj) override
	{
		...
		ScriptEngine::Start_Timer(obj, this, 4.0f, STATIONARY);
		ScriptEngine::Start_Timer(obj, this, 3.5f, WAVE_ANIM);
	...
	void Action_Complete (GameObject *obj, int action_id, ActionCompleteReason reason) override
	{
		if (action_id == DOING_ANIMATION)
		{
			...
			params.Set_Basic( this, 96, ENGINEER_GOTO );
			params.Set_Movement( ScriptEngine::Get_Position (ScriptEngine::Find_Object (1200026)), RUN, 0.8f );

			ScriptEngine::Action_Goto (obj, params);

			ScriptEngine::Send_Custom_Event( obj, obj, 100005, 0, 0.6f );
		}
		...
		if((action_id == ENGINEER_GOTO) && (action_id != ScriptEngine::Get_Action_ID(obj)) && (!ScriptEngine::Is_Performing_Pathfind_Action(obj)) && (reason == ACTION_COMPLETE_LOW_PRIORITY || reason == ACTION_COMPLETE_PATH_BAD_DEST))
		{
			ScriptEngine::Start_Timer (obj, this, 5.0f, RESEND_GOTO);
		}
		...
		{
			ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (1200001), ENGINEER_CROUCH, 0, 0.0f );

			ScriptEngine::Lock_Soldier_Facing (obj, STAR, true );

			/*Vector3 direction = ScriptEngine::Get_Position (STAR) - ScriptEngine::Get_Position (obj);
			float angle = RAD_TO_DEGF(WWMath::Atan2(direction.Y,direction.X));
			ScriptEngine::Set_Facing ( obj, angle );*/
			ActionParamsStruct params;
	...
	void Timer_Expired(GameObject * obj, int timer_id) override
	{
		...
		{
			GameObject * sniper1 = ScriptEngine::Find_Object (sniper_1);
		...
		/*if (timer_id == 990 && ScriptEngine::Find_Object (1200017))
		{
			ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (1200001), 100007, 0, 0.0f );
		}

		if (timer_id == 989 && ScriptEngine::Find_Object (1200017) && count == 5)
		{
			//ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (1200001), 100006, 0, 0.0f );
		}*/
	...
	void Custom(GameObject * obj, int type, intptr_t param, GameObject * /* sender */) override
	{
		...
		{
			sniper_1 = param;
			ScriptEngine::Start_Timer (obj, this, 5.0f, CROUCH_GOTO);
		}
		...
		if (type == RUN_AWAY)
		{
			...
			params.Set_Basic( this, 97, RUN_AWAY );
			params.Set_Movement( ScriptEngine::Find_Object (1200007), RUN, 1.0f, false );
			ScriptEngine::Action_Goto ( obj, params );
		}
		...
		if (type == SNIPER_ANIM)
		{
			...
			ScriptEngine::Action_Reset(obj, 100);
			ScriptEngine::Set_Animation (obj, "H_A_A0A0_L53", false, nullptr, 0.0F, -1.0F, true );
		...
		if (type == KILL)
		{
			...
			params.Set_Basic( this, 95, 1 );
			params.Set_Movement( ScriptEngine::Find_Object(1200022), 1.2f, 1.0f, true );
			params.Set_Attack (ScriptEngine::Find_Object(1200017), 250.0f, 0.0f, 1);
			params.AttackCheckBlocked = true;
			ScriptEngine::Action_Attack( obj, params );
		}
...
DECLARE_SCRIPT(MX0_Engineer_Return, "")
{
	...
	void Entered (GameObject * obj, GameObject * /* enterer */) override
	{
		if (!already_entered)
		{
			...
			ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (1200001), ENGINEER_RETURN, 1, 0.0f );
		}
	}
};
...
DECLARE_SCRIPT(MX0_Engineer_Goto, "GotoDest1:int, GotoDest2:int, Count:int")
{
	...
	void Custom (GameObject *obj, int type, intptr_t param, GameObject * /* sender */) override
	{
		...
		{
			ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (param), LEAD, Get_Int_Parameter("GotoDest1"), 0.0f );
		}

		if (type == RETURN_ENGINEER2)
		{
			ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (param), LEAD, Get_Int_Parameter("GotoDest2"), 0.0f );
		}
	}
	...
	void Entered (GameObject * obj, GameObject * /* enterer */) override
	{
		if (!already_entered)
		{
			...
			ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (1200001), COUNTER, Get_Int_Parameter("Count"), 0.0f );

			ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (1200001), CHECK_ENGINEER, 1, 0.0f );
			ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (1200001), CHECK_ENGINEER, 2, 0.0f );

			/*GameObject * engineer = ScriptEngine::Find_Object (Get_Int_Parameter("GotoObject"));

			ScriptEngine::Send_Custom_Event( obj, engineer, LEAD, Get_Int_Parameter("GotoDest"), 0.0f );*/
		}
	}
};
...
DECLARE_SCRIPT(MX0_Engineer_Goto2, "GotoDest1:int, GotoDest2:int, Count:int")
{
	...
	void Custom (GameObject *obj, int type, intptr_t param, GameObject * /* sender */) override
	{
		...
		if (type == SEND_EM)
		{
			ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (1200001), RUNNING_CONV, 1, 0.0f );
	...
	void Timer_Expired (GameObject* /* obj */, int timer_id) override
	{
		...
		{
			ScriptEngine::Set_HUD_Help_Text(MX0_HELPTEXT_04, TEXT_COLOR_OBJECTIVE_PRIMARY);
		}
	}
	...
	void Entered (GameObject * obj, GameObject * /* enterer */) override
	{
		if (!already_entered)
		{
			if (Get_Int_Parameter("Count") != 5)
			{
				...
				//ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (1200001), 100004, 1, 0.0f );
			}
			...
			if (Get_Int_Parameter("Count") == 5)
			{
				...
				ScriptEngine::Start_Timer(obj, this, 3.0f, FIRE_HELP);
				ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (1200001), CROUCH_WANDER, 0, 0.0f );

				ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (1200001), 100006, 1, 0.0f );
			}
		}
	}
};
...
DECLARE_SCRIPT(MX0_Kill_Sniper, "")
{
	...
	void Entered (GameObject * obj, GameObject * /* enterer */) override
	{
		if (!already_entered)
		{
			...
			if (ScriptEngine::Find_Object (1200017))
			{
				ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (1200001), KILL_SNIPER, 0, 0.0f );
			}
		}
	}
};
...
DECLARE_SCRIPT(MX0_NOD_TroopDrop, "")
{
	...
	void Timer_Expired (GameObject* /* obj */, int timer_id) override
	{
		...
		{
			Vector3 drop_loc = ScriptEngine::Get_Position ( ScriptEngine::Find_Object(1200018));
			float chin_face = ScriptEngine::Get_Facing ( ScriptEngine::Find_Object(1200018));

			GameObject * chinook_obj = ScriptEngine::Create_Object ( "Invisible_Object", drop_loc);
			ScriptEngine::Set_Facing(chinook_obj, chin_face);
			ScriptEngine::Attach_Script(chinook_obj, "Test_Cinematic", "MX0_C130Troopdrop.txt");

			drop_loc = ScriptEngine::Get_Position ( ScriptEngine::Find_Object(1200020));
			chin_face = ScriptEngine::Get_Facing ( ScriptEngine::Find_Object(1200020));

			GameObject * chinook_obj2 = ScriptEngine::Create_Object ( "Invisible_Object", drop_loc);
			ScriptEngine::Set_Facing(chinook_obj2, chin_face);
			ScriptEngine::Attach_Script(chinook_obj2, "Test_Cinematic", "MX0_C130Troopdrop2.txt");
		}
	}
	...
	void Entered (GameObject * obj, GameObject * /* enterer */) override
	{
		if (!already_entered)
		{
			...
			ScriptEngine::Start_Timer(obj, this, 3.0f, TROOP_DROP);

			ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (1200001), START_SNIPER, 0, 0.0f );
		}
	}
};
...
DECLARE_SCRIPT (MX0_AmbientBattle, "")
{
	...
	void Timer_Expired (GameObject* obj, int timer_id) override
	{
		...
		{
			battle_sound = ScriptEngine::Create_Sound ( "MX0_AmbientBattle_DME", ScriptEngine::Get_Position(obj), obj );
			ScriptEngine::Monitor_Sound (obj, battle_sound);
		}
	}
	...
	void Custom (GameObject * obj, int type, intptr_t param, GameObject * /* sender */) override
	{
		...
		{
			ScriptEngine::Start_Timer(obj, this, 3.0f, AMB_SOUND);
		}
	}
};
...
DECLARE_SCRIPT (MX0_SniperAction, "FaceObj:int")
{
	...
	void Custom (GameObject * obj, int type, intptr_t /* param */, GameObject * /* sender */) override
	{
		if(type == START_SNIPER)
		{
			ScriptEngine::Start_Timer(obj, this, 5.0f, SNIPE);
		}
	}
	...
	void Damaged( GameObject * obj, GameObject * damager, float /* amount */ ) override
	{
		...
		if ( damager == STAR )
		{
			ScriptEngine::Apply_Damage( obj, 10000.0f, "Blamokiller", STAR );

			bool reveal = ScriptEngine::Reveal_Encyclopedia_Character( 41 );
			if ( reveal == 1 )
			{
	...
	void Timer_Expired (GameObject* obj, int timer_id) override
	{
		if (timer_id == SNIPE)
		{
			...
			params.Set_Attack (ScriptEngine::Find_Object(Get_Int_Parameter("FaceObj")), 250.0f, 0.0f, 1);
...
DECLARE_SCRIPT (MX0_KillNotify, "")
{
	...
	void Custom (GameObject *obj, int type, intptr_t /* param */, GameObject * /* sender */) override
	{
		if(type == START_SNIPER)
		{
			ScriptEngine::Start_Timer(obj, this, 3.0f, SNIPE_CONV);
			ScriptEngine::Start_Timer(obj, this, 7.0f, SNIPE_CONV2);
		}
	}
	...
	void Timer_Expired (GameObject* obj, int timer_id) override
	{
		...
		{
			int id = ScriptEngine::Create_Conversation("MX0CON020", 95, 2000, false);
		...
		{
			int id = ScriptEngine::Create_Conversation("MX0CON021", 99, 2000, false);
			ScriptEngine::Join_Conversation (obj, id, true, true, true );
			ScriptEngine::Start_Conversation(id, 100021);
			ScriptEngine::Monitor_Conversation(obj, id);

			ScriptEngine::Start_Timer(obj, this, 5.0f, SNIPE_CONV_DONE);
		}
		...
		if (timer_id == SNIPE_CONV_DONE)
		{
			if (hit)
			{
				...
				int rand_conv_snipe = ScriptEngine::Get_Random_Int (0, 6);

				int id = ScriptEngine::Create_Conversation(snipe_conv[rand_conv_snipe], 95, 2000, false);
				ScriptEngine::Join_Conversation (obj, id, true, true, true );
				ScriptEngine::Start_Conversation(id, 100021);
				ScriptEngine::Monitor_Conversation(obj, id);
	...
	void Killed (GameObject *obj, GameObject *killer) override
	{
		if (killer == STAR)
		{
			ScriptEngine::Send_Custom_Event( obj, ScriptEngine::Find_Object (1200001), SNIPER1KILLED, 0, 0.0f );
		}
	}
};
...
DECLARE_SCRIPT(DAK_MX0_Sec_3_Humvee, "" )
{
	void Entered ( GameObject * /* obj */, GameObject * /* enterer */ ) override
	{
		...
		GameObject *humvee = ScriptEngine::Find_Object( 1400035 ); // gets pointer to section 3 Humvee
		ScriptEngine::Action_Goto( humvee, params );
		ScriptEngine::Debug_Message( "***** DAK_MX0_Sec_3_Humvee:Entered code exectued.\n" );
	}
};
...
DECLARE_SCRIPT(MX0_A03_NOD_PLACED_MINIGUNNER, "" )
{
	void Created( GameObject *obj ) override
	{
		Vector3 home_pos = ScriptEngine::Get_Position( obj );
		ScriptEngine::Set_Innate_Soldier_Home_Location( obj, home_pos, 5);
		// disable innate
		ScriptEngine::Innate_Disable( obj );
		ScriptEngine::Set_Innate_Is_Stationary(obj, true);
	}
...
DECLARE_SCRIPT(MX0_GDI_ORCA, "" )
{
	...
	void Action_Complete(GameObject * obj, int action_id, ActionCompleteReason reason) override
	{
		if(reason == ACTION_COMPLETE_CONVERSATION_ENDED)
		{
			if ( action_id == 1 )
			{
				...
				GameObject * orca = ScriptEngine::Create_Object ( "Invisible_Object", Vector3 (0, 0, 0));
				ScriptEngine::Attach_Script ( orca, "Test_Cinematic", "X0F_Harvester.txt" );
				// send custom to self to fire off orca "spotted harvester" conversation
				ScriptEngine::Send_Custom_Event( obj, obj, 1, 0, 1.5 );
				// send custom to harvester to damage self ( 1st time )
				GameObject *harvester = ScriptEngine::Find_Object( MX0_A03_NOD_HARVESTER_ID );
				ScriptEngine::Send_Custom_Event( obj, harvester, MX0_A03_CUSTOM_NOD_HARVESTER_DMG_SELF, 4, (1 + 198/30) );
				// send custom to harvester to damage self ( 2nd time )
				ScriptEngine::Send_Custom_Event( obj, harvester, MX0_A03_CUSTOM_NOD_HARVESTER_DMG_SELF, 3, (1 + 260/30) );
				// send custom to harvester to damage self ( 3rd time )
				ScriptEngine::Send_Custom_Event( obj, harvester, MX0_A03_CUSTOM_NOD_HARVESTER_DMG_SELF, 3, (1 + 272/30) );
				// send custom to self to fire off orca "finished and departing" conversation
				ScriptEngine::Send_Custom_Event( obj, obj, 2, 0, 9.5 );
			}
		}
	}
	...
	void Entered ( GameObject *obj, GameObject * /* enterer */ ) override
	{
		if (first_time)
		{
			...
			GameObject *Tank_Drop = ScriptEngine::Find_Object ( MX0_A03_GDI_TANK_DROP_ID );
			if ( Tank_Drop )
			{
				// ScriptEngine::Debug_Message( "***** DAK ***** sending custom, dropping Medium tank\n" );
				ScriptEngine::Attach_Script ( Tank_Drop, "Test_Cinematic", "XG_A03_Tank_Drop.txt" );
			}
	...
	void Custom( GameObject *obj, int type, intptr_t param, GameObject * /* sender */ ) override
	{
		...
		{
			// get Trooper 1
			GameObject *controller = ScriptEngine::Find_Object( MX0_A03_CONTROLLER_ID );
			if ( controller )
			{
				ScriptEngine::Send_Custom_Event( obj, controller , M00_SEND_OBJECT_ID, 12, 0 );
			}
			GameObject *Trooper_One = ScriptEngine::Find_Object( Trooper_One_Id );

			// "Trooper1: Woah. "That Rocked!" Orca6: "This is Orca 6 -- bingo Fuel. Returning to Base..."
			int conv_id = ScriptEngine::Create_Conversation ( "MX0_A03_04" , 0, 0, true);
			ScriptEngine::Join_Conversation(Trooper_One, conv_id, false, false, true);
			ScriptEngine::Join_Conversation(nullptr, conv_id, true, true, true);
			...
			if (Trooper_One)
			{
				ScriptEngine::Send_Custom_Event( obj, Trooper_One, 5, 0, 0 );
			}

			// send custom to trooper1 to fire off "Blast that harvester for us sir!" after 10 seconds.
			if (Trooper_One)
			{
				ScriptEngine::Send_Custom_Event( obj, Trooper_One, 3, 0, 10 );
			}
		}
...
DECLARE_SCRIPT(MX0_NOD_INFANTRY, "troop_num:int")
{
	...
	void Created ( GameObject *obj ) override
	{
		...
		if ( controller )
		{
			ScriptEngine::Send_Custom_Event( obj, controller, MX0_A03_CUSTOM_NOD_LEDGE_CREATED, 0, 0);
		}

		if (troop_num == 1 )
		{
			ScriptEngine::Attach_Script( obj, "M00_Send_Object_ID", "1400041,15,0"); // sending id to A03 Controller
		}
		else
		{
			ScriptEngine::Attach_Script( obj, "M00_Send_Object_ID", "1400041,17,0"); // sending id to A03 Controller
		}
	}
	...
	void Animation_Complete(GameObject * obj, const char *anim) override
	{
		...
		{
			ScriptEngine::Send_Custom_Event( obj, obj, 1, 0, 0.33f );
		}
	}
	...
	void Custom ( GameObject *obj, int type, intptr_t /* param */, GameObject *target ) override
	{
		...
		if (type == 1) // you've landed! move to your position.
		{
			...
			if (troop_num == 1 ) // move to left pos.
			{
				...
				// ScriptEngine::Debug_Message( "***** DAK ***** Setting NOD Ledge Troop 1 to move to left position.\n" );
				params.Set_Movement( ScriptEngine::Find_Object( MX0_A03_NOD_LEDGE_LEFT_LOC_ID ), RUN, 0.25f );
				ScriptEngine::Action_Goto (obj, params);
			...
			else // move to right pos.
			{
				ActionParamsStruct params;
				params.Set_Basic(this, INNATE_PRIORITY_ENEMY_SEEN, 0);
				// ScriptEngine::Debug_Message( "***** DAK ***** Setting NOD Ledge Troop 2 to move to right position.\n" );
				params.Set_Movement( ScriptEngine::Find_Object( MX0_A03_NOD_LEDGE_RIGHT_LOC_ID ), RUN, 0.25f );
				ScriptEngine::Action_Goto (obj, params);
			}
		}
	}
	...
	void Killed ( GameObject *obj, GameObject * /* killer */ ) override
	{
		...
		if ( controller )
		{
			ScriptEngine::Send_Custom_Event( obj, controller, MX0_A03_CUSTOM_NOD_LEDGE_KILLED, troop_num, 2);
		}
	}
};
...
DECLARE_SCRIPT(MX0_A03_GDI_INFANTRY, "troop_num:int" )
{
	...
	void Created ( GameObject *obj ) override
	{
		...
		// disable inate
		// ScriptEngine::Innate_Disable (obj);
		ScriptEngine::Attach_Script( obj, "M00_Damage_Modifier_DME", "0.15,0,1,1,1"); // Setting Damage Modifier.
		...
		if ( troop_num == 1 )
		{
			...
			ScriptEngine::Attach_Script( obj, "M00_Send_Object_ID", "1400041,3,0"); // sending first Drop Troop id to A03 Controller

			// move to your Position
			ScriptEngine::Action_Reset( obj, 100 );
			ActionParamsStruct params;
			params.Set_Basic ( this, 91, 0 );
			params.Set_Movement( ScriptEngine::Find_Object( MX0_A03_DROP_TROOP_ONE_LOC_ID ), RUN, 0.25f );
			ScriptEngine::Action_Goto (obj, params);
		}
		...
		if ( troop_num == 2 )
		{
			...
			// send id to controller.
			ScriptEngine::Attach_Script( obj, "M00_Send_Object_ID", "1400041,5,0"); // sending second Drop Troop id to A03 Controller.

			// move to your Position
			ScriptEngine::Action_Reset( obj, 100 );
			ActionParamsStruct params;
			params.Set_Basic ( this, 91, 0 );
			params.Set_Movement( ScriptEngine::Find_Object( MX0_A03_DROP_TROOP_TWO_LOC_ID ), RUN, 0.25f );
			ScriptEngine::Action_Goto (obj, params);
		}
		...
		if ( troop_num == 3 )
		{
			...
			// send id to controller.
			ScriptEngine::Attach_Script( obj, "M00_Send_Object_ID", "1400041,7,0"); // sending third Drop Troop id to A03 Controller.
			...
			params.Set_Movement( ScriptEngine::Find_Object( MX0_A03_DROP_TROOP_THREE_LOC_ID ), RUN, 0.25f );
			ScriptEngine::Action_Goto (obj, params);
		}
	}
	void Action_Complete( GameObject *obj, int action_id, ActionCompleteReason /* complete_reason */) override
	...
	void Custom ( GameObject *obj, int type, intptr_t /* param */, GameObject * /* sender */ ) override
	{
		...
		{
			GameObject *target = ScriptEngine::Find_Object( Current_Target );
...
DECLARE_SCRIPT(MX0_A03_GDI_TROOP_DROP_ZONE_DAK, "" )
{
	...
	void Entered ( GameObject * /* obj */, GameObject * /* enterer */ ) override
	{
		if ( First_Time )
		{
			GameObject *Troop_Drop =  ScriptEngine::Find_Object ( MX0_A03_GDI_TROOP_DROP_ID );
			if ( Troop_Drop )
			{
				// ScriptEngine::Debug_Message( "***** sending custom, dropping 3 infantry\n" );
				ScriptEngine::Attach_Script ( Troop_Drop, "Test_Cinematic", "MX0_A03_GDI_TroopDrop.txt" );
			}
			First_Time = false;
		}
	}
...
DECLARE_SCRIPT(MX0_A03_GDI_TANK_DROP_ZONE_DAK, "" )
{
	...
	void Entered ( GameObject * /* obj */, GameObject * /* enterer */ ) override
	{
		if ( First_Time )
		{
			...
			if ( Tank_Drop )
			{
				// ScriptEngine::Debug_Message( "***** sending custom, dropping Medium tank\n" );
				ScriptEngine::Attach_Script ( Tank_Drop, "Test_Cinematic", "XG_A03_Tank_Drop.txt" );
			}
			First_Time = false;
		}
	...
	void Custom ( GameObject *obj, int type, intptr_t param, GameObject *sender ) override
	{
		if ( type == 0 )
		{
			...
			if ( buggie_Dead && harvester_Dead )
			{
				...
				{
					ScriptEngine::Send_Custom_Event( obj, obj, MX0_A03_CUSTOM_TYPE_END_ZONE, 0, 0);
					GameObject *end_zone = ScriptEngine::Find_Object (MX0_A03_END_ZONE_ID );
					ScriptEngine::Send_Custom_Event( obj, end_zone, MX0_A03_CUSTOM_LOCK_END_ZONE, 0, 0 );
				}
			}
		}
		...
		if ( type == MX0_A03_CUSTOM_NOD_LEDGE_KILLED )
		{
			...
			{
				GameObject * minigunners = ScriptEngine::Create_Object ( "Invisible_Object", ScriptEngine::Get_Position( ScriptEngine::Find_Object ( MX0_A03_NOD_LEDGE_DROP_LOC_ID )));
				ScriptEngine::Attach_Script ( minigunners, "Test_Cinematic", "MX0_A03_NOD_LedgeDrop.txt" );
				num_drops++;
			}
		}
		...
		if ( type == MX0_A03_CUSTOM_TYPE_END_ZONE )
		{
			...
			GameObject *tank = ScriptEngine::Find_Object( GDI_Drop_Tank_Id );
			if ( tank )
			{
				ScriptEngine::Action_Reset( tank, 100 );

				ActionParamsStruct params;
				params.Set_Basic ( this, 91, 1 );
				params.Set_Movement( Vector3(0,0,0), 1.0f, 1 );
				params.WaypathID = MX0_A03_WAYPATH_TANK_ID;
				ScriptEngine::Action_Goto (tank, params);
			}
			...
			if (humvee)
			{
				ScriptEngine::Send_Custom_Event( obj, humvee, 3, 0, 1.25f);
			}
			...
			if ( trooper_one )
			{
				ScriptEngine::Send_Custom_Event( obj, trooper_one, 2, 0, 0);
				//ScriptEngine::Debug_Message( "***** DAK ***** custom sent to trooper 1.\n" );
			}
		}
		...
		{
			ScriptEngine::Scale_AI_Awareness( 2, 1 );

			// create humvee
			GameObject *humvee =  ScriptEngine::Find_Object ( MX0_A03_HUMVEE_DROP_ID );
			if ( humvee )
			{
				ScriptEngine::Debug_Message( "***** sending custom, dropping humvee, attaching script\n" );
				ScriptEngine::Attach_Script ( humvee, "Test_Cinematic", "XG_A03_HumveeDrop_B.txt" );
			}
			// begin moving NOD Buggie
			GameObject *buggie = ScriptEngine::Find_Object ( MX0_A03_NOD_BUGGIE_ID );
			if ( buggie )
			{
				ScriptEngine::Send_Custom_Event( obj, buggie, 1, 0, 0 );
			}
			...
			if (trooper_one)
			{
				ScriptEngine::Attach_Script( trooper_one, "MX0_A03_GDI_TROOPER_ONE", "" );
				// move trooper1 up to first A03 position.
				ScriptEngine::Send_Custom_Event( obj, trooper_one, 1, 0, 2);
			}

			// enable placed minigunners

			GameObject *minigunner_one = ScriptEngine::Find_Object ( MX0_A03_NOD_PLACED_MINIGUNNER_ONE_ID);
			if (minigunner_one)
			{
				ScriptEngine::Send_Custom_Event( obj, minigunner_one, MX0_A03_CUSTOM_NOD_PLACED_MINIGUNNER_INNATE_ENABLE, 0, 0);
			}

			GameObject *minigunner_two = ScriptEngine::Find_Object ( MX0_A03_NOD_PLACED_MINIGUNNER_TWO_ID);
			if (minigunner_two)
			{
				ScriptEngine::Send_Custom_Event( obj, minigunner_two, MX0_A03_CUSTOM_NOD_PLACED_MINIGUNNER_INNATE_ENABLE, 0, 0);
			}
		}
...
/*			if ( zone_not_finished )
			{
				...
				int rand = ScriptEngine::Get_Random_Int( 1, 2 );
				if ( rand == 1 )
				{
					// Trooper1: "Standing in Tiberium...Not Smart.
					int conv_id = ScriptEngine::Create_Conversation ( "MX0_A03_06" , 0, 0, true);
					ScriptEngine::Join_Conversation( Trooper_One, conv_id, false, false, true);
				...
				else
				{
					...
					int conv_id = ScriptEngine::Create_Conversation ( "MX0_A03_07" , 0, 0, true);
					ScriptEngine::Join_Conversation( Trooper_One, conv_id, false, false, true);
					ScriptEngine::Start_Conversation (conv_id, 0);
				}
			}
*/
		...
		if ( type == MX0_A03_CUSTOM_HARVESTER_DEAD )
		{
			...
			if ( zone_not_finished )
			{
				if (Trooper_One)
				{
					...
					int conv_id = ScriptEngine::Create_Conversation ( "MX0_A03_08" , 0, 0, true);
					ScriptEngine::Join_Conversation( Trooper_One, conv_id, false, false, true);
					ScriptEngine::Start_Conversation (conv_id, 0);
				}
			}
			...
			if (Trooper_One)
			{
				ScriptEngine::Send_Custom_Event( obj, Trooper_One, 4, 0, 0 );
			}
		...
		if ( type == M00_SEND_OBJECT_ID )
		{
			if ( param == 1 )
			{
				Humvee_Id = ScriptEngine::Get_ID( sender ); // get humvee Id.
			}
			if ( param == 2 ) // send humvee Id.
			{
				ScriptEngine::Send_Custom_Event( obj, sender, M00_SEND_OBJECT_ID, Humvee_Id, 0 );
			}
			if ( param == 3 )
			{
				GDI_Drop_Troop_One_Id = ScriptEngine::Get_ID( sender ); // get GDI_Drop_Troop_One Id.
			}
			if ( param == 4 )
			{
				ScriptEngine::Send_Custom_Event( obj, sender, M00_SEND_OBJECT_ID, GDI_Drop_Troop_One_Id, 0 );
			}
			if ( param == 5 )
			{
				GDI_Drop_Troop_Two_Id = ScriptEngine::Get_ID( sender ); // get GDI_Drop_Troop_Two Id.
			}
			if ( param == 6 )
			{
				ScriptEngine::Send_Custom_Event( obj, sender, M00_SEND_OBJECT_ID, GDI_Drop_Troop_Two_Id, 0 );
			}
			if ( param == 7 )
			{
				GDI_Drop_Troop_Three_Id = ScriptEngine::Get_ID( sender ); // get GDI_Drop_Troop_Three Id.
			}
			if ( param == 8 )
			{
				ScriptEngine::Send_Custom_Event( obj, sender, M00_SEND_OBJECT_ID, GDI_Drop_Troop_Three_Id, 0 );
			}
			if ( param == 9 )
			{
				GDI_Drop_Tank_Id = ScriptEngine::Get_ID( sender ); // get GDI_Drop_Tank_Id Id.
			}
			if ( param == 10 )
			{
				ScriptEngine::Send_Custom_Event( obj, sender, M00_SEND_OBJECT_ID, GDI_Drop_Tank_Id, 0 );
			}
			if ( param == 11 )
			{
				Trooper_One_Id = ScriptEngine::Get_ID( sender );
				ScriptEngine::Debug_Message( "***** DAK ***** A03: recieved trooper1 ID.\n" );
			}
			if ( param == 12 )
			{
				ScriptEngine::Send_Custom_Event( obj, sender, M00_SEND_OBJECT_ID, Trooper_One_Id, 0 );
			}
			if ( param == 13 )
			{
				Havoc_Tank_Id = ScriptEngine::Get_ID( sender );
				ScriptEngine::Attach_Script( obj, "M00_Send_Object_ID", "1500017,10,1.0f"); // sending Havoc_Tank_Id to shuman.
			}
			if ( param == 14 )
			{
				ScriptEngine::Send_Custom_Event( obj, sender, M00_SEND_OBJECT_ID, Havoc_Tank_Id, 0 );
			}
			if ( param == 15 )
			{
				Nod_Ledge_One_Id = ScriptEngine::Get_ID( sender );
			}
			if ( param == 16 )
			{
				ScriptEngine::Send_Custom_Event( obj, sender, M00_SEND_OBJECT_ID, Nod_Ledge_One_Id, 0 );
			}
			if ( param == 17 )
			{
				Nod_Ledge_Two_Id = ScriptEngine::Get_ID( sender );
			}
			if ( param == 18 )
			{
				ScriptEngine::Send_Custom_Event( obj, sender, M00_SEND_OBJECT_ID, Nod_Ledge_Two_Id, 0 );
			}
		}
	}
};
...
DECLARE_SCRIPT ( MX0_A03_HUMVEE, "" ) // moves humvee
{
	...
	void Created( GameObject *obj ) override
	{
		...
		// send id to controller.
		ScriptEngine::Attach_Script( obj, "M00_Send_Object_ID", "1400041,1,0"); // sending humvee id to A03 Controller.

		// send id to shuman fool.
		ScriptEngine::Attach_Script( obj, "M00_Send_Object_ID", "1500017,1,1.0f"); // sending humvee id to shuman
		ScriptEngine::Attach_Script( obj, "M00_Send_Object_ID", "1500020,1,1.0f");
	...
	void Action_Complete( GameObject *obj, int action_id, ActionCompleteReason /* complete_reason */) override
	{
		...
		{
			// ScriptEngine::Debug_Message( "***** DAK ***** finding Object Buggie.\n" );
			Current_Target = Target_Id[target];
			GameObject *current_target = ScriptEngine::Find_Object( Current_Target );
	...
	void Custom ( GameObject *obj, int type, intptr_t /* param */, GameObject * /* sender */ ) override
	{
		if ( type == 1 ) // halt fire for 1 - 3 seconds.
		{
			...
			ScriptEngine::Modify_Action( obj, 1, params, false, true );

			float delay = ScriptEngine::Get_Random( 1, 3 );
			ScriptEngine::Send_Custom_Event( obj, obj, 2, 0, delay );
		}
		...
		if ( type == 2 ) // resume fire on buggy. halt after 3 - 6 seconds.
		{
			...
			if ( current_target )
			{
				...
				float delay = ScriptEngine::Get_Random( 3, 6 );
				ScriptEngine::Send_Custom_Event( obj, obj, 1, 0, delay );
			}
		}
		if ( type == 3 )
		{
			ScriptEngine::Debug_Message( "***** DAK ***** Humvee: Recieved custom to move to section 4.\n" );
...
DECLARE_SCRIPT ( MX0_A03_TANK, "" ) // moves tank
{
	...
	void Created( GameObject *obj ) override
	{
		...
		// send id to controller.
		ScriptEngine::Attach_Script( obj, "M00_Send_Object_ID", "1400041,9,0"); // sending humvee id to A03 Controller.

		// send Id to shuman
		ScriptEngine::Attach_Script( obj, "M00_Send_Object_ID", "1500017,3,1.0f"); // sending tank id to A03 Controller.
		ScriptEngine::Attach_Script( obj, "M00_Send_Object_ID", "1500020,3,1.0f"); // sending tank id to A03 Controller.
		...
		// destroy NOD Turrets.
		ScriptEngine::Send_Custom_Event ( obj, obj, 0, 0, 1); // attack Turret_One
	}
	void Action_Complete ( GameObject *obj, int action_id, ActionCompleteReason /* complete_reason */ ) override
	{
		...
		{
			target = target + 1;
			ScriptEngine::Send_Custom_Event ( obj, obj, 0, 0, 1); // attack next target.
		}
	}
...
DECLARE_SCRIPT ( MX0_A03_NOD_BUGGIE, "" )
{
	...
	void Killed ( GameObject *obj, GameObject * /* killer */ ) override
	{
		...
		GameObject *nod_trooper = ScriptEngine::Create_Object("Nod_Minigunner_0", position);
		ScriptEngine::Attach_Script( nod_trooper, "MX0_A03_NOD_TROOPER_TIB_DEATH", "" );
	}
};
...
DECLARE_SCRIPT ( MX0_A03_NOD_TROOPER_TIB_DEATH, "" )
{
	...
	void Killed ( GameObject *obj, GameObject * /* killer */ ) override
	{
		...
		if ( controller )
		{
			ScriptEngine::Send_Custom_Event( obj, controller, MX0_A03_CUSTOM_BUGGIE_DEAD, 0, 2);
		}
	}
...
DECLARE_SCRIPT ( MX0_A03_NOD_HARVESTER, "" )
{
	...
	void Created ( GameObject *obj ) override
	{
		// ScriptEngine::Attach_Script( obj, "M00_Damage_Modifier_DME", "0.25,1,1,0,0" );
		ScriptEngine::Set_Health(obj, 1200);
	...
	void Custom( GameObject *obj, int type, intptr_t /* param */, GameObject * /* sender */ ) override
	{
		if ( type == MX0_A03_CUSTOM_NOD_HARVESTER_DMG_SELF )
		{
			...
			// ScriptEngine::Send_Custom_Event( obj, obj, M00_ENABLE_DAMAGE_MOD, 0, 0 );
			// apply damage to yourself based on param.
			// ScriptEngine::Debug_Message( "***** DAK ***** Orcastrike: Damaging harvester.\n" );
			// float health = ScriptEngine::Get_Health( obj );
			// float damage = health / param;
			ScriptEngine::Apply_Damage( obj, 200, "EXPLOSIVE", nullptr );
		}
	}
	void Killed( GameObject *obj, GameObject * /* killer */ ) override
	{
		...
		if ( controller )
		{
			ScriptEngine::Send_Custom_Event( obj, controller , MX0_A03_CUSTOM_HARVESTER_DEAD, 0, 0 );
		}
	}
};
...
DECLARE_SCRIPT( MX0_A03_GDI_TROOPER_ONE, "" )
{
	...
	void Created( GameObject *obj ) override
	{
		ScriptEngine::Attach_Script( obj, "M00_Send_Object_ID", "1500017,3,1.0f"); // sending Trooper_One_Id to shuman.
		ScriptEngine::Innate_Disable( obj );
	...
	void Custom( GameObject *obj, int type, intptr_t /* param */, GameObject * /* sender */ ) override
	{
		...
		if (type == 1)
		{
			// ScriptEngine::Debug_Message( "***** DAK ***** Trooper One: Custom Recieved. Moving along MX0_A03_WAYPATH_GDI_TROOPER_ONE_A_ID\n" );
			ScriptEngine::Action_Reset( obj, 100 );
	...
	void Custom( GameObject *obj, int type, intptr_t /* param */, GameObject * /* sender */ ) override
	{
		if (type == CUSTOM_EVENT_VEHICLE_ENTERED )
		{
			if (first_time) // first time player enters tank. send trooper 1 ahead.
			{
				...
				if ( controller )
				{
					// ScriptEngine::Debug_Message( "***** sending custom MX0_A03_CUSTOM_TYPE_START_ZONE\n" );
					ScriptEngine::Send_Custom_Event ( obj, controller, MX0_A03_CUSTOM_TYPE_START_ZONE, 0, 0 );
				}
				first_time = false;
			}
		}
...
DECLARE_SCRIPT( MX0_A03_FIRST_PLAYER_ZONE, "" )
{
	...
	void Entered ( GameObject *obj, GameObject * /* enterer */ ) override
	{
		if (first_time)
		{
			...
			if (trooper_one)
			{
				ScriptEngine::Action_Reset( trooper_one, 100 );
				// send custom to move trooper one along MX0_A03_WAYPATH_GDI_TROOPER_ONE_B_ID
				ScriptEngine::Send_Custom_Event( obj, trooper_one, 0, 0, 0.0f);
			}
...
DECLARE_SCRIPT( MX0_A03_TROOPER_ONE_TEST, "" )
{
	void Created (GameObject *obj ) override
	{
		ScriptEngine::Attach_Script( obj, "M00_Send_Object_ID", "1400041,11,0.0f"); // sending Trooper_One_Id to A03_Controller
	}
};
...
DECLARE_SCRIPT ( MX0_A03_NOD_TURRET, "" )
{
	void Created ( GameObject *obj ) override
	{
		...
		if ( controller )
		{
			ScriptEngine::Send_Custom_Event( obj, controller, MX0_A03_CUSTOM_NOD_TURRET_CREATED, 0, 0);
		}
	}
	...
	void Killed ( GameObject *obj, GameObject * /* killer */ ) override
	{
		...
		if ( controller )
		{
			ScriptEngine::Send_Custom_Event( obj, controller, MX0_A03_CUSTOM_NOD_TURRET_DESTROYED, 0, 0);
		}
	}
};
...
DECLARE_SCRIPT ( MX0_A03_END_ZONE, "" )
{
	...
	void Entered( GameObject *obj, GameObject * /* enterer */ ) override
	{
		if ( first_time )
		{
			...
			if ( controller )
			{
				ScriptEngine::Send_Custom_Event( obj, controller, MX0_A03_CUSTOM_TYPE_END_ZONE, 1, 0);
			}
			first_time = false;
		}
	}
