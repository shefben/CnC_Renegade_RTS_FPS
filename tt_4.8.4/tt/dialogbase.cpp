...
////////////////////////////////////////////////////////////////
REF_DEF1(DialogBaseClass::DefaultCmdHandler, DEFAULT_DLG_CMD_HANDLER, 0x0082FE5C);
...
DialogBaseClass::Start_Dialog (void)
{
	...
	DialogParserClass::Parse_Template (DialogResID, &dlg_width, &dlg_height,
								&Title, &control_list, &dlg_exstyle);
	...
	if (screen_rect.Width() / screen_rect.Height() <= RES_SCREEN_WIDTH / RES_SCREEN_HEIGHT)
	{
		// smaller than 4:3, use width as primary
		screen_ar_width = screen_rect.Width();
		screen_ar_height = screen_ar_width * RES_SCREEN_HEIGHT / RES_SCREEN_WIDTH;
	}
	else
	{
		// larger than 4:3, use height as primary
		screen_ar_height = screen_rect.Height();
		screen_ar_width =  screen_ar_height * RES_SCREEN_WIDTH / RES_SCREEN_HEIGHT;
	}

	int dlg_screen_width				= int(((float)dlg_width / RES_SCREEN_WIDTH) * screen_ar_width);
	int dlg_screen_height				= int(((float)dlg_height / RES_SCREEN_HEIGHT) * screen_ar_height);

	//
	//	Center the dialog on the screen
	//
	if (dlg_exstyle & WS_EX_RIGHT)
	{
		Rect.Left = (float)(int(screen_rect.Right - dlg_screen_width));
		Rect.Top = (float)(int(screen_rect.Center().Y - ((float)dlg_screen_height * 0.5F)));
		Rect.Right	= (float)(int(Rect.Left + dlg_screen_width));
		Rect.Bottom	= (float)(int(Rect.Top + dlg_screen_height));
	}
	else
	{
		Rect.Left	= (float)(int(screen_rect.Center ().X - ((float)dlg_screen_width * 0.5F)));
		Rect.Top		= (float)(int(screen_rect.Center ().Y - ((float)dlg_screen_height * 0.5F)));
		Rect.Right	= (float)(int(Rect.Left + dlg_screen_width));
		Rect.Bottom	= (float)(int(Rect.Top + dlg_screen_height));
	}
	...
	for (int index = 0; index < control_list.Count (); index ++) {
		...
		if (control != NULL) {
			...
			int ctrl_width		= int((((float)info.cx) / RES_SCREEN_WIDTH) * screen_ar_width);
			int ctrl_height	= int((((float)info.cy) / RES_SCREEN_HEIGHT) * screen_ar_height);

			//
			//	Calculate the screen position of the control
			//
			RectClass rect;
			rect.Left	= (float)(int(Rect.Left + ((((float)info.x) / RES_SCREEN_WIDTH) * screen_ar_width)));
			rect.Top		= (float)(int(Rect.Top + ((((float)info.y) / RES_SCREEN_HEIGHT) * screen_ar_height)));
			rect.Right	= (float)(int(rect.Left + ctrl_width));
			rect.Bottom	= (float)(int(rect.Top + ctrl_height));
...
DialogBaseClass::Update_Mouse_State (void)
{
	...
	//
	const Vector3 &last_mouse_pos = DialogMgrClass::Get_Last_Mouse_Pos ();
	if ((last_mouse_pos.Z != cursor_pos.Z) && (DialogMgrClass::Get_Active_Dialog() != NULL)) {
		DialogMgrClass::Get_Active_Dialog()->On_Mouse_Wheel ((int)(last_mouse_pos.Z - cursor_pos.Z));
	}
...
DialogBaseClass::Set_Rect (const RectClass &rect)
{
	...
	for (int index = 0; index < ChildDialogList.Count (); index ++) {
		...
		//	Offset this dialog
		//
		RectClass rectx = ChildDialogList[index]->Get_Rect ();
		rectx.Left	+= offset.X;
		rectx.Right	+= offset.X;
		rectx.Top		+= offset.Y;
		rectx.Bottom	+= offset.Y;
		ChildDialogList[index]->Set_Rect (rectx);
	}
