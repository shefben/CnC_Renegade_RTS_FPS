...
void SSGMGameManager::FreeData()
{
	...
	{
		delete[] GDISpawnChar;
		GDISpawnChar = nullptr;
	}
	if (NodSpawnChar)
	{
		delete[] NodSpawnChar;
		NodSpawnChar = nullptr;
	}
...
PurchaseStatus SSGMGameManager::PowerupPurchaseHook(BaseControllerClass *base,GameObject *purchaser,unsigned int cost,unsigned int preset,const char *data)
{
	...
	{
		PurchaseStatus ret = RegisteredEvents[EVENT_POWERUP_PURCHASE_HOOK][i]->OnPowerupPurchase(base,purchaser,cost,preset,data);
		if (ret == PurchaseStatus_AllowFree)
		{
			fp = true;
		}
		else if (ret != PurchaseStatus_Allow)
		{
			return ret;
		}
	}
	if (fp)
	{
		return PurchaseStatus_AllowFree;
	}
	return PurchaseStatus_Allow;
}

PurchaseStatus SSGMGameManager::VehiclePurchaseHook(BaseControllerClass *base,GameObject *purchaser,unsigned int cost,unsigned int preset,const char *data)
{
	...
	{
		PurchaseStatus ret = RegisteredEvents[EVENT_VEHICLE_PURCHASE_HOOK][i]->OnVehiclePurchase(base,purchaser,cost,preset,data);
		if (ret == PurchaseStatus_AllowFree)
		{
			fp = true;
		}
...
PurchaseStatus SSGMGameManager::CharacterPurchaseHook(BaseControllerClass *base,GameObject *purchaser,unsigned int cost,unsigned int preset,const char *data)
{
	SSGM_Soldier *script = (SSGM_Soldier *)Find_Script_On_Object(purchaser,"SSGM_Soldier");
	...
	{
		PurchaseStatus ret = RegisteredEvents[EVENT_CHARACTER_PURCHASE_HOOK][i]->OnCharacterPurchase(base,purchaser,cost,preset,data);
		if (ret == PurchaseStatus_AllowFree)
		{
			fp = true;
		}
