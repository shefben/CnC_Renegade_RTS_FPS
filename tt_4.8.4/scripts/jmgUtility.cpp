...
void JMG_Utility_Zone_Damage_While_In_Zone::Created(GameObject *obj)
{
	soldierMotorcycleTracked = Get_Vector3_Parameter("Soldier|Motorcycle|Tracked");
	vtolWheeledUnused = Get_Vector3_Parameter("VTOL|Wheeled|Unused");
	sprintf(warhead,"%s",Get_Parameter("Warhead"));
	sprintf(message,"%s",Get_Parameter("Message"));
	messageRgb = Get_Vector3_Parameter("MessageRgb");
	delay = Get_Float_Parameter("Delay");
}
void JMG_Utility_Zone_Damage_While_In_Zone::Entered(GameObject *obj,GameObject *enterer)
{
	if (!Is_Script_Attached(enterer,"JMG_Utility_Zone_Damage_While_In_Zone_Attached"))
	{
		if (enterer->As_SoldierGameObj() && soldierMotorcycleTracked.X != 0.0f)
			AttachScript(enterer,soldierMotorcycleTracked.X);
		else if (enterer->As_VehicleGameObj() && enterer->As_PhysicalGameObj()->Peek_Physical_Object()->As_MotorcycleClass() && soldierMotorcycleTracked.Y != 0.0f)
			AttachScript(enterer,soldierMotorcycleTracked.Y);
		else if (enterer->As_VehicleGameObj() && enterer->As_PhysicalGameObj()->Peek_Physical_Object()->As_TrackedVehicleClass() && soldierMotorcycleTracked.Z != 0.0f)
			AttachScript(enterer,soldierMotorcycleTracked.Z);
		else if (enterer->As_VehicleGameObj() && enterer->As_PhysicalGameObj()->Peek_Physical_Object()->As_VTOLVehicleClass() && vtolWheeledUnused.X != 0.0f)
			AttachScript(enterer,vtolWheeledUnused.X);
		else if (enterer->As_VehicleGameObj() && enterer->As_PhysicalGameObj()->Peek_Physical_Object()->As_WheeledVehicleClass() && vtolWheeledUnused.Y != 0.0f)
			AttachScript(enterer,vtolWheeledUnused.Y);
	}
	if (Is_Script_Attached(enterer,"JMG_Utility_Zone_Damage_While_In_Zone_Attached"))
	{
		JMG_Utility_Zone_Damage_While_In_Zone_Attached *script = (JMG_Utility_Zone_Damage_While_In_Zone_Attached*)Find_Script_On_Object(enterer,"JMG_Utility_Zone_Damage_While_In_Zone_Attached");
		if (script)
			script->zoneCount++;
	}
}
void JMG_Utility_Zone_Damage_While_In_Zone::Exited(GameObject *obj,GameObject *exiter)
{
	if (!Is_Script_Attached(exiter,"JMG_Utility_Zone_Damage_While_In_Zone_Attached"))
		return;
	JMG_Utility_Zone_Damage_While_In_Zone_Attached *script = (JMG_Utility_Zone_Damage_While_In_Zone_Attached*)Find_Script_On_Object(exiter,"JMG_Utility_Zone_Damage_While_In_Zone_Attached");
	if (script)
	{
		script->zoneCount--;;
		if (script->zoneCount <= 0)
			Remove_Script(exiter,"JMG_Utility_Zone_Damage_While_In_Zone_Attached");
	}
}
void JMG_Utility_Zone_Damage_While_In_Zone::AttachScript(GameObject *obj,float damage)
{
	char params[256];
	sprintf(params,"%.2f,%s,%.2f,%s,%.2f %.2f %.2f",damage,warhead,delay,message,messageRgb.X,messageRgb.Y,messageRgb.Z);
	Attach_Script_Once(obj,"JMG_Utility_Zone_Damage_While_In_Zone_Attached",params);
}
void JMG_Utility_Zone_Damage_While_In_Zone_Attached::Created(GameObject *obj)
{
	damage = Get_Float_Parameter("Damage");
	sprintf(warhead,"%s",Get_Parameter("Warhead"));
	delay = Get_Float_Parameter("Delay");
...
void JMG_Utility_Zone_Damage_While_In_Zone_Attached::Timer_Expired(GameObject *obj,int number)
{
	if (number == 1)
	{
		Commands->Apply_Damage(obj,damage,warhead,obj);
	...
	if (number == 2)
	{
		...
		if (driver && Commands->Is_A_Star(driver))
			Set_HUD_Help_Text_Player_Text(driver,7234,message,messageRgb);
		else if (Commands->Is_A_Star(obj) && !Get_Vehicle(obj))
			Set_HUD_Help_Text_Player_Text(obj,7234,message,messageRgb);
		Commands->Start_Timer(obj,this,3.0f,2);
	}
}
void JMG_Utility_Zone_Damage_While_In_Zone_Attached::Detach(GameObject *obj)
{
	if (_stricmp(message,""))
		Set_HUD_Help_Text_Player_Text(obj,7234,"",messageRgb);
}
void JMG_Utility_Visible_Vehicle_Occupants::Created(GameObject *obj)
{
	sprintf(driverPreset,"%s",Get_Parameter("DriverPreset"));
	sprintf(seatBoneName,"%s",Get_Parameter("SeatBoneName"));
	sprintf(driverAnimation,"%s",Get_Parameter("DriverAnimation"));
	sprintf(passengerAnimation,"%s",Get_Parameter("PassengerAnimation"));
	damageable = Get_Int_Parameter("Damageable") ? true : false;
	debug = Get_Int_Parameter("Debug") ? true : false;
}
void JMG_Utility_Visible_Vehicle_Occupants::Custom(GameObject *obj,int message,int param,GameObject *sender) 
{
	if (message == CUSTOM_EVENT_VEHICLE_ENTERED)
	{
		if (debug)
			for (int x = 0;x < Get_Vehicle_Seat_Count(obj);x++)
				AddOccupant(obj,sender,x);
		else
			AddOccupant(obj,sender,Get_Occupant_Seat(obj,sender));
	}
	if (message == CUSTOM_EVENT_VEHICLE_EXITED)
	{
		...
		for (int x = 0;x < 64;x++)
		{
			if (occupantActualIds[x] != senderId)
				continue;
			GameObject *visible = Commands->Find_Object(occupantVisibleIds[x]);
			if (!visible)
				return;
			if (damageable)
			{
				Commands->Set_Health(sender,Commands->Get_Health(visible));
				Commands->Set_Shield_Strength(sender,Commands->Get_Shield_Strength(visible));
			}
			Commands->Destroy_Object(visible);
		}
	}
}
void JMG_Utility_Visible_Vehicle_Occupants::Destroyed(GameObject *obj)
{
	for (int x = 0;x < 64;x++)
	{
		GameObject *visible = Commands->Find_Object(occupantVisibleIds[x]);
...
void JMG_Utility_Visible_Vehicle_Occupants::AddOccupant(GameObject *obj,GameObject *sender,int seatId)
{
	GameObject *visible = Commands->Create_Object(driverPreset,Vector3(0.0f,0.0f,0.0f));
	Commands->Disable_Physical_Collisions(visible);
	Commands->Set_Model(visible,Get_Model(sender));
	if (damageable)
	{
		Set_Skin(visible,Get_Skin(sender));
		Commands->Set_Shield_Type(visible,Get_Shield_Type(sender));
		Set_Max_Health(visible,Commands->Get_Max_Health(sender));
		Commands->Set_Health(visible,Commands->Get_Health(sender));
		Set_Max_Shield_Strength(visible,Commands->Get_Max_Shield_Strength(sender));
		Commands->Set_Shield_Strength(visible,Commands->Get_Shield_Strength(sender));
	}
	char seatBone[32];
	sprintf(seatBone,"%s%0d",seatBoneName,seatId);
	Commands->Attach_To_Object_Bone(visible,obj,seatBone);
	Commands->Set_Animation(visible,!seatId ? driverAnimation : passengerAnimation,true,0,0.0f,-1.0f,true);
	occupantActualIds[seatId] = Commands->Get_ID(sender);
	occupantVisibleIds[seatId] = Commands->Get_ID(visible);
	char params[512];
	sprintf(params,"%d",Commands->Get_ID(sender));
	Commands->Attach_Script(visible,"JMG_Utility_Visible_Vehicle_Occupants_Attached",params);
}
void JMG_Utility_Visible_Vehicle_Occupants_Attached::Killed(GameObject *obj,GameObject *killer)
{
	GameObject *owner = Commands->Find_Object(Get_Int_Parameter("OwnerId"));
	if (!owner)
		return;
	GameObject *vehicle = Get_Vehicle(owner);
	if (!vehicle)
		return;
	Force_Occupant_Exit(vehicle,Get_Occupant_Seat(vehicle,owner));
	Commands->Apply_Damage(owner,99999.9f,"BlamoKiller",killer);
	char params[220];
	sprintf(params,"%d",Commands->Get_ID(killer));
	Commands->Attach_Script(owner,"JMG_Utility_Visible_Vehicle_Occupants_Attached_Kill_Cleanup",params);

}
void JMG_Utility_Visible_Vehicle_Occupants_Attached_Kill_Cleanup::Created(GameObject *obj)
{
	Commands->Start_Timer(obj,this,0.0f,1);
}
void JMG_Utility_Visible_Vehicle_Occupants_Attached_Kill_Cleanup::Timer_Expired(GameObject *obj,int number)
{
	if (number == 1)
	{
		if (Get_Vehicle(obj))
		...
		Commands->Apply_Damage(obj,99999.9f,"BlamoKiller",Commands->Find_Object(Get_Int_Parameter("KillerID")));
	}
}
void JMG_Utility_Zone_Disable_Specific_Weapon_Presets::Created(GameObject *obj)
{
	sprintf(presets,"%s",Get_Parameter("Presets"));
	delim = Get_Parameter("Delim")[0];
	sprintf(message,"%s",Get_Parameter("Message"));
	messageRgb = Get_Vector3_Parameter("MessageRgb");
}
void JMG_Utility_Zone_Disable_Specific_Weapon_Presets::Entered(GameObject *obj,GameObject *enterer)
{
	if (!Is_Script_Attached(enterer,"JMG_Utility_Zone_Disable_Specific_Weapon_Presets_Attached"))
	{
		char params[512];
		sprintf(params,"%s,%c,%s,%.2f %.2f %.2f",presets,delim,message,messageRgb.X,messageRgb.Y,messageRgb.Z);
		Attach_Script_Once(enterer,"JMG_Utility_Zone_Disable_Specific_Weapon_Presets_Attached",params);
	}
	if (Is_Script_Attached(enterer,"JMG_Utility_Zone_Disable_Specific_Weapon_Presets_Attached"))
	{
		JMG_Utility_Zone_Disable_Specific_Weapon_Presets_Attached *script = (JMG_Utility_Zone_Disable_Specific_Weapon_Presets_Attached*)Find_Script_On_Object(enterer,"JMG_Utility_Zone_Disable_Specific_Weapon_Presets_Attached");
		if (script)
			script->zoneCount++;
	}
}
void JMG_Utility_Zone_Disable_Specific_Weapon_Presets::Exited(GameObject *obj,GameObject *exiter)
{
	if (!Is_Script_Attached(exiter,"JMG_Utility_Zone_Disable_Specific_Weapon_Presets_Attached"))
		return;
	JMG_Utility_Zone_Disable_Specific_Weapon_Presets_Attached *script = (JMG_Utility_Zone_Disable_Specific_Weapon_Presets_Attached*)Find_Script_On_Object(exiter,"JMG_Utility_Zone_Disable_Specific_Weapon_Presets_Attached");
	if (script)
	{
		script->zoneCount--;;
		if (script->zoneCount <= 0)
			Remove_Script(exiter,"JMG_Utility_Zone_Disable_Specific_Weapon_Presets_Attached");
	}
}
void JMG_Utility_Zone_Disable_Specific_Weapon_Presets_Attached::Created(GameObject *obj)
{
	holding = false;
	char delim[1];
	sprintf(delim,"%s",Get_Parameter("Delim"));
	char presets[2048];
	sprintf(presets,"%s",Get_Parameter("Presets"));
	char *token = strtok(presets,delim);
	while(token != NULL)
	{
		char tok[128];
		sprintf(tok,"%s",token);
		weaponPresets.Add_Tail(new StringNode(tok));
		token = strtok(NULL,delim);
	}
	sprintf(message,"%s",Get_Parameter("Message"));
	messageRgb = Get_Vector3_Parameter("MessageRgb");
...
void JMG_Utility_Zone_Disable_Specific_Weapon_Presets_Attached::Timer_Expired(GameObject *obj,int number)
{
	if (number == 1)
	{
		holding = false;
		const char *weap = Get_Current_Weapon(obj);
		for (SLNode<StringNode> *current = weaponPresets.Head();current;current = current->Next())
		{
			if (weap && !_stricmp(weap,current->Data()->preset))
			{
				int clip = Get_Clip_Bullets(obj,weap);
				int bullets = Get_Bullets(obj,weap);
				if (clip || bullets)
				{
					current->Data()->bullets += bullets;
					current->Data()->clip += clip;
					Set_Bullets(obj,weap,0);
					Set_Clip_Bullets(obj,weap,0);
				}
				holding = true;
				break;
			}
...
void JMG_Utility_Zone_Disable_Specific_Weapon_Presets_Attached::Detach(GameObject *obj)
{
	if (_stricmp(message,""))
		Set_HUD_Help_Text_Player_Text(obj,7234," ",messageRgb);
	Destroyed(obj);
}
void JMG_Utility_Zone_Disable_Specific_Weapon_Presets_Attached::Destroyed(GameObject *obj)
{
	for (SLNode<StringNode> *current = weaponPresets.Head();current;current = current->Next())
	{
		if (!current->Data()->clip && !current->Data()->bullets)
			continue;
		Set_Clip_Bullets(obj,current->Data()->preset,current->Data()->clip);
		Set_Bullets(obj,current->Data()->preset,current->Data()->bullets);
		current->Data()->bullets = 0;
		current->Data()->clip = 0;
		holding = false;
	}
	weaponPresets.Remove_All();
}
void JMG_Utility_Zone_Damage_While_In_Zone_Presets::Created(GameObject *obj)
{
	char delim[1];
	sprintf(delim,"%s",Get_Parameter("Delim"));
	char presetList[2048];
	sprintf(presetList,"%s",Get_Parameter("Presets"));
	char *token = strtok(presetList,delim);
	while(token != NULL)
	{
		char tok[128];
		sprintf(tok,"%s",token);
		presets.Add_Tail(new StringNode(tok));
		token = strtok(NULL,delim);
	}
	damage = Get_Float_Parameter("Damage");
	sprintf(warhead,"%s",Get_Parameter("Warhead"));
...
void JMG_Utility_Zone_Damage_While_In_Zone_Presets::Entered(GameObject *obj,GameObject *enterer)
{
	if (!Is_Script_Attached(enterer,"JMG_Utility_Zone_Damage_While_In_Zone_Presets_Attached"))
	{
		const char *preset = Commands->Get_Preset_Name(enterer);
		for (SLNode<StringNode> *current = presets.Head();current;current = current->Next())
		{
			if (!_stricmp(preset,current->Data()->preset))
			{
				char params[256];
				sprintf(params,"%.2f,%s,%.2f,%s,%.2f %.2f %.2f",damage,warhead,delay,message,messageRgb.X,messageRgb.Y,messageRgb.Z);
				Attach_Script_Once(enterer,"JMG_Utility_Zone_Damage_While_In_Zone_Presets_Attached",params);
				return;
			}
		}
	}
	if (Is_Script_Attached(enterer,"JMG_Utility_Zone_Damage_While_In_Zone_Presets_Attached"))
	{
		JMG_Utility_Zone_Damage_While_In_Zone_Presets_Attached *script = (JMG_Utility_Zone_Damage_While_In_Zone_Presets_Attached*)Find_Script_On_Object(enterer,"JMG_Utility_Zone_Damage_While_In_Zone_Presets_Attached");
		if (script)
			script->zoneCount++;
	}
}
void JMG_Utility_Zone_Damage_While_In_Zone_Presets::Exited(GameObject *obj,GameObject *exiter)
{
	if (!Is_Script_Attached(exiter,"JMG_Utility_Zone_Damage_While_In_Zone_Presets_Attached"))
		return;
	JMG_Utility_Zone_Damage_While_In_Zone_Presets_Attached *script = (JMG_Utility_Zone_Damage_While_In_Zone_Presets_Attached*)Find_Script_On_Object(exiter,"JMG_Utility_Zone_Damage_While_In_Zone_Presets_Attached");
	if (script)
	{
		script->zoneCount--;;
		if (script->zoneCount <= 0)
			Remove_Script(exiter,"JMG_Utility_Zone_Damage_While_In_Zone_Presets_Attached");
	}
}
void JMG_Utility_Zone_Damage_While_In_Zone_Presets::Destroyed(GameObject *obj)
{
	presets.Remove_All();
}
void JMG_Utility_Zone_Damage_While_In_Zone_Presets_Attached::Created(GameObject *obj)
{
	damage = Get_Float_Parameter("Damage");
	sprintf(warhead,"%s",Get_Parameter("Warhead"));
	delay = Get_Float_Parameter("Delay");
...
void JMG_Utility_Zone_Damage_While_In_Zone_Presets_Attached::Timer_Expired(GameObject *obj,int number)
{
	if (number == 1)
	{
		Commands->Apply_Damage(obj,damage,warhead,obj);
...
void JMG_Utility_Zone_Damage_While_In_Zone_Presets_Attached::Detach(GameObject *obj)
{
	if (_stricmp(message,""))
		Set_HUD_Help_Text_Player_Text(obj,7234," ",messageRgb);
}
...
ScriptRegistrant<JMG_Utility_Zone_Damage_While_In_Zone> JMG_Utility_Zone_Damage_While_In_Zone_Registrant("JMG_Utility_Zone_Damage_While_In_Zone","Soldier|Motorcycle|Tracked=1.0 1.0 1.0:vector3,VTOL|Wheeled|Unused=1.0 1.0 1.0:vector3,Warhead=None:string,Delay=0.1:float,Message:string,MessageRgb:vector3");
ScriptRegistrant<JMG_Utility_Zone_Damage_While_In_Zone_Attached> JMG_Utility_Zone_Damage_While_In_Zone_Attached_Registrant("JMG_Utility_Zone_Damage_While_In_Zone_Attached","Damage:float,Warhead:string,Delay:float,Message:string,MessageRgb:vector3");
ScriptRegistrant<JMG_Utility_Visible_Vehicle_Occupants> JMG_Utility_Visible_Vehicle_Occupants_Registrant("JMG_Utility_Visible_Vehicle_Occupants","DriverPreset=Simple_Sydney_SandM_Wall:string,SeatBoneName:string,DriverAnimation:string,PassengerAnimation:string,Damageable:int,Debug:int");
ScriptRegistrant<JMG_Utility_Visible_Vehicle_Occupants_Attached> JMG_Utility_Visible_Vehicle_Occupants_Attached_Registrant("JMG_Utility_Visible_Vehicle_Occupants_Attached","OwnerId:int");
ScriptRegistrant<JMG_Utility_Visible_Vehicle_Occupants_Attached_Kill_Cleanup> JMG_Utility_Visible_Vehicle_Occupants_Attached_Kill_Cleanup_Registrant("JMG_Utility_Visible_Vehicle_Occupants_Attached_Kill_Cleanup","KillerId:int");
ScriptRegistrant<JMG_Utility_Zone_Disable_Specific_Weapon_Presets> JMG_Utility_Zone_Disable_Specific_Weapon_Presets_Registrant("JMG_Utility_Zone_Disable_Specific_Weapon_Presets","Presets:string,Delim:string,Message:string,MessageRgb:vector3");
ScriptRegistrant<JMG_Utility_Zone_Disable_Specific_Weapon_Presets_Attached> JMG_Utility_Zone_Disable_Specific_Weapon_Presets_Attached_Registrant("JMG_Utility_Zone_Disable_Specific_Weapon_Presets_Attached","Presets:string,Delim:string,Message:string,MessageRgb:vector3");
ScriptRegistrant<JMG_Utility_Zone_Damage_While_In_Zone_Presets> JMG_Utility_Zone_Damage_While_In_Zone_Presets_Registrant("JMG_Utility_Zone_Damage_While_In_Zone_Presets","Presets:string,Delim:string,Damage:float,Warhead=None:string,Delay=0.1:float,Message:string,MessageRgb:vector3");
ScriptRegistrant<JMG_Utility_Zone_Damage_While_In_Zone_Presets_Attached> JMG_Utility_Zone_Damage_While_In_Zone_Presets_Attached_Registrant("JMG_Utility_Zone_Damage_While_In_Zone_Presets_Attached","Damage:float,Warhead:string,Delay:float,Message:string,MessageRgb:vector3");
