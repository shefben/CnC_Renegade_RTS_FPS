...
public:
	...
	static void Give_Points(GameObject *player,float points)
	{
		int playerId = Get_Player_ID(player);
		if (!playerId)
			return;
		Set_Score(playerId,Get_Score(playerId)+points);
	}
};
...
/*!
* \brief Applies damage to the selected class(s) of vehicle or soldier while inside the zone(s)
* \Soldier|Motorcycle|Tracked - How much damage to apply to each type, if 0 nothing happens to that type
* \VTOL|Wheeled|Unused - How much damage to apply to each type, if 0 nothing happens to that type
* \Warhead - warhead to use
* \Delay - delay between each damage effect
* \Message - Message to display on the hud while in the zone
* \MessageRgb - Color of the displayed message RGB (0-1)
* \author jgray
* \ingroup JmgUtility
*/
class JMG_Utility_Zone_Damage_While_In_Zone : public ScriptImpClass {
	Vector3 soldierMotorcycleTracked;
	Vector3 vtolWheeledUnused;
	char warhead[128];
	float delay;
	char message[220];
	Vector3 messageRgb;
	...
	void Entered(GameObject *obj,GameObject *enterer);
	void Exited(GameObject *obj,GameObject *exiter);
	void AttachScript(GameObject *obj,float damage);
};

/*!
* \brief Attached by JMG_Utility_Zone_Damage_While_In_Zone
* \author jgray
* \ingroup JmgUtility
*/
class JMG_Utility_Zone_Damage_While_In_Zone_Attached : public ScriptImpClass {
	float damage;
	char warhead[128];
	float delay;
	char message[220];
...
/*!
* \brief Adds a visible (and potentially damageable) prop as a driver or passenger to the vehicle
* \DriverPreset - Object that gets replaced with the player's model and health (I recommend a simple object that is not targetable and have blammo heath by default)
* \SeatBoneName - Base name for the seat bone to attach to (IE FakeSeat would give you: FakeSeat0 FakeSeat1 etc)
* \DriverAnimation - Animation to play for the driver
* \PassengerAnimation - Animation to play for the passengers 
* \Damageable - Can the occupants be hurt
* \Debug - When in enabled all occupants will be visible when one person enters, useful for debugging 
* \author jgray
* \ingroup JmgUtility
*/
class JMG_Utility_Visible_Vehicle_Occupants : public ScriptImpClass {
	char driverPreset[64];
	char seatBoneName[16];
	char driverAnimation[32];
	char passengerAnimation[32];
	bool damageable;
	float debug;
	int occupantActualIds[64];
	int occupantVisibleIds[64];
	void Created(GameObject *obj);
	void Custom(GameObject *obj,int message,int param,GameObject *sender);
	void Destroyed(GameObject *obj);
	void AddOccupant(GameObject *obj,GameObject *sender,int seatId);
public:
	JMG_Utility_Visible_Vehicle_Occupants()
	{
		for (int x = 0;x < 64;x++)
		{
			occupantActualIds[x] = 0;
			occupantVisibleIds[x] = 0;
		}
	}
};

/*!
* \brief Attached by JMG_Utility_Visible_Vehicle_Occupants
* \author jgray
* \ingroup JmgUtility
*/
class JMG_Utility_Visible_Vehicle_Occupants_Attached : public ScriptImpClass
{
  void Killed(GameObject *obj,GameObject *killer);
};
...
* \ingroup JmgUtility
*/
class JMG_Utility_Visible_Vehicle_Occupants_Attached_Kill_Cleanup : public ScriptImpClass
{
	void Created(GameObject *obj);
	void Timer_Expired(GameObject *obj,int number);
};
...
/*!
* \brief Disables all weapons that are listed (try not to have multiple of this script on the same zone, it'll have odd issues) by setting their ammo to 0 and then restoring it when you leave
* \Presets - a list of presets separated by the delim (IE: POW_Pistol_Player@CnC_Weapon_AutoRifle_Player_Nod@CnC_Weapon_APC_M60MG_RedTracer)
* \Delim - the character used to split the preset string, don't use ,
* \Message - Message to display while holding the beacon
* \MessageRgb - Color of the displayed message RGB (0-1)
* \author jgray
* \ingroup JmgUtility
*/
class JMG_Utility_Zone_Disable_Specific_Weapon_Presets : public ScriptImpClass {
	char presets[1024];
	char delim;
	char message[220];
	Vector3 messageRgb;
	...
	void Exited(GameObject *obj,GameObject *exiter);
public:
	JMG_Utility_Zone_Disable_Specific_Weapon_Presets()
	{
		sprintf(presets,"");
		delim = ' ';
		sprintf(message,"");
...
/*!
* \brief Attached by JMG_Utility_Zone_Disable_Specific_Weapon_Presets
* \author jgray
* \ingroup JmgUtility
*/
class JMG_Utility_Zone_Disable_Specific_Weapon_Presets_Attached : public ScriptImpClass {
	struct StringNode
	{
		char preset[128];
		int bullets;
	...
	bool holding;
	SList<StringNode> weaponPresets;
	char message[220];
	Vector3 messageRgb;
	void Created(GameObject *obj);
	void Timer_Expired(GameObject *obj,int number);
...
/*!
* \brief Like JMG_Utility_Zone_Damage_While_In_Zone but only affects certain presets
* \Presets - a list of presets separated by the delim (IE: Nod_Jet@CnC_Nod_Buggy@CnC_Nod_Minigunner_0)
* \Delim - the character used to split the preset string, don't use ,
* \Damage - how much damage to do to the specified vehicles
* \Warhead - warhead to use
* \Delay - delay between each damage effect
* \Message - Message to display on the hud while in the zone
* \MessageRgb - Color of the displayed message RGB (0-1)
...
* \ingroup JmgUtility
*/
class JMG_Utility_Zone_Damage_While_In_Zone_Presets : public ScriptImpClass {
	struct StringNode
	{
		char preset[128];
		StringNode(const char *preset)
...
/*!
* \brief Attached by JMG_Utility_Zone_Damage_While_In_Zone_Presets
* \author jgray
* \ingroup JmgUtility
*/
class JMG_Utility_Zone_Damage_While_In_Zone_Presets_Attached : public ScriptImpClass {
	float damage;
	char warhead[128];
	float delay;
	char message[220];
