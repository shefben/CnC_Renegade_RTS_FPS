...
wchar_t *StrDupW(const wchar_t *pSrc){
	...
	if (!pSrc/* || !pSrc[0]*/) return NULL;
	len = StrLenW(pSrc);
	pResult = (wchar_t*)Malloc((len+1)*sizeof(wchar_t));
	if (!pResult) return NULL;
...
wchar_t *StrnDupW(const wchar_t *pStr, int len){
	...
	len = (_len < len) ? _len : len;
	if (pStr[len-1] != 0) len++;
	pRes = (wchar_t*)Malloc((_len = len)*sizeof(wchar_t));
	while (--_len) *(pRes++) = *(pStr++);
	*pRes = 0;
	return pRes-(len-1);
}
...
file fopen_W(const wchar_t *_filename, const wchar_t *_mode){
	...
	if ((hFile = CreateFileW(_filename, acc, share, NULL, disp, 0, NULL)) == INVALID_FILE) return INVALID_FILE;
	if (is_append) SetFilePointer(hFile, 0, NULL, SEEK_END);
	return hFile;
}
...
int TokenizeA(char *pSrc, char **dest_array, int max_count, const char *extra_ws, const char *extra_quotation, BOOL extended_whitespace, BOOL translate_slashes, BOOL leave_quotation, BOOL ignore_empty){
	...
	while (1){
		switch (*pSrc){
			...
			case '\t':
			case ' ':
				if (extra_ws) goto __process_char;
__process_ws:
				if (!In_Quotes) {
					pSrc++;
					if (count || !ignore_empty){
						*pTemp = 0;
						dest_array[token++] = pCur;
						/*
