...
unsigned int MD5File (char *fn, unsigned long seed, MD5_CTX *mdContext, unsigned int size)
{
	char buf[2048];
	unsigned int trb=0;
	FILE *f = fopen(fn,"rb");
	...
	for (;;) {
		unsigned int rb = (unsigned int)fread(buf,1,2048,f);
...
unsigned int MD5String (char *s, unsigned long seed, MD5_CTX *mdContext, unsigned int size)
{
	if (size > 0) {
		MD5Init(mdContext,seed);
		MD5Update(mdContext,(unsigned char *)s, size);
		MD5Final(mdContext);
	}
	return size;
}

char *MD5Char(char *out, MD5_CTX *mdContext) {
	unsigned int i;
	char *s = out;
	for (i=0; i<16; i++, s+=2) sprintf (s, "%02x", mdContext->digest[i]) ;
	return out;
}
